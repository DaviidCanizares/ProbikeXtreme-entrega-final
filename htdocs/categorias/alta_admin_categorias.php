...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        // Cambiamos el estado de la categoría a 'activo'
        $sql = "UPDATE categories SET estado = 'activo' WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);

        // Redirigimos con mensaje de éxito
        header('Location: categorias_admin_general.php?message=Categoría activada correctamente');
        exit();
    } catch (PDOException $e) {
        die("Error al activar la categoría: " . $e->getMessage());
    }
} else {
